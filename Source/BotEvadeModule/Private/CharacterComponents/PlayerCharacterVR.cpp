...
DEFINE_LOG_CATEGORY(LogPlayerCharacterVR);
...
APlayerCharacterVR::APlayerCharacterVR()
{
    ...
    LeftMotionController = CreateDefaultSubobject<UMotionControllerComponent>("Left Controller");
    LeftMotionController->SetTrackingSource(EControllerHand::Left);
    LeftControllerVisualization = CreateDefaultSubobject<UXRDeviceVisualizationComponent>("Left Controller Visualization");
    LeftControllerVisualization->SetupAttachment(LeftMotionController);

    RightMotionController = CreateDefaultSubobject<UMotionControllerComponent>("Right Controller");
...
void APlayerCharacterVR::BeginPlay()
{
    ...
    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Left Controller Name: %s"), *LeftMotionController->GetName());
    UE_LOG(LogPlayerCharacterVR,
           Warning,
           TEXT("Left Controller Motion Source: %s"),
           *LeftMotionController->GetTrackingMotionSource().ToString());
    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Left Controller Full Name: %s"), *LeftMotionController->GetFullName());
    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Left Visualization Name: %s"), *LeftControllerVisualization->GetName());
    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Left Visualization Full Name: %s"), *LeftControllerVisualization->GetFullName());
    ...
    if (const APlayerController* PlayerController = Cast<APlayerController>(Controller))
    {
            ...
            UEnhancedInputLocalPlayerSubsystem>(PlayerController->GetLocalPlayer()))
        {
            ...
            UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Unable to add mapping context"));
    }
    else
        UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Unable to get the PlayerController"));

    if (!UHeadMountedDisplayFunctionLibrary::EnableHMD(true))
    {
        UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Unable to activate HMD"));
        return;
    }

    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("HMD Activated: %s"), *UHeadMountedDisplayFunctionLibrary::GetHMDDeviceName().ToString());
    ...
    UE_LOG(LogPlayerCharacterVR, Warning, TEXT("Left Controller Device Name: %s"), *MotionControllerData.DeviceName.ToString());
...
void APlayerCharacterVR::SmoothTurn(const FInputActionValue& Value)
{
...
void APlayerCharacterVR::SnapTurn(const FInputActionValue& Value)
{
    if (EnableSmoothRotation)
    {
        return;
...
void APlayerCharacterVR::ToggleCrouch(const FInputActionValue& Value)
{
...
void APlayerCharacterVR::PerformJump(const FInputActionValue& Value)
{
...
void APlayerCharacterVR::GrabAxisLeft(const float AxisValue) const
{
    // TODO: Add hand animation
}

void APlayerCharacterVR::GrabAxisRight(const float AxisValue) const
{
    // TODO: Add hand animation
}

void APlayerCharacterVR::UpdateRoomScaleLocation()
{
...
void APlayerCharacterVR::UpdateCapsuleHeight()
{
    FRotator Rotation;
    FVector Position;
