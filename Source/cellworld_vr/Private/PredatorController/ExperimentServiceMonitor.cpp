...
bool AExperimentServiceMonitor::SpawnAndPossessPredator() {
	...
	// Spawn the character
	this->PredatorBasic = GetWorld()->SpawnActor<APredatorBasic>(APredatorBasic::StaticClass(), SpawnVector, Rotation,
	                                                             SpawnParams);

	// Ensure the character was spawned
	if (!this->PredatorBasic) {
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::SpawnAndPossessPredator()] Spawn APredatorBasic Failed!"));
		return false;
	}
...
bool AExperimentServiceMonitor::StopExperiment(const FString& ExperimentNameIn) {
	if (!this->ValidateClient(TrackingClient)) {
		UE_LOG(LogExperiment, Error,
		       TEXT(
			       "[AExperimentServiceMonitor::StopExperiment] Can't stop experiment, Experiment Service client not valid."
		       ));
		return false;
	}
...
bool AExperimentServiceMonitor::ResetTrackingAgent() {
	...
	if (!ensureMsgf(ResetRequest, TEXT("[AExperimentServiceMonitor::ResetTrackingAgent] ResetRequest is NULL"))) { return false; }
...
void AExperimentServiceMonitor::OnTimedOut(const FString InMessage) {
	UE_LOG(LogExperiment, Fatal, TEXT("[AExperimentServiceMonitor::OnTimedOutDelegate] %s"), *InMessage);
	ResetWorldState();
...
void AExperimentServiceMonitor::OnSubscribeResult(const bool bSubscribeResult) {
	...
	if (bSubscribeResult) { UE_LOG(LogExperiment, Log, TEXT("Subscription successful!")) }
	else { UE_LOG(LogExperiment, Error, TEXT("Subscription Failed!")) }
}
...
void AExperimentServiceMonitor::HandleStartEpisodeRequestTimedOut() {
	UE_LOG(LogExperiment, Error,
	       TEXT("[AExperimentServiceMonitor::HandleStartEpisodeRequestTimedOut] Episode request timed out!"))
...
void AExperimentServiceMonitor::UpdatePreyPosition(const FVector InVector, const FRotator InRotation) {
	...
	if (!ensure(this->ValidateClient(TrackingClient))) {
		UE_LOG(LogExperiment, Warning, TEXT("[AExperimentServiceMonitor::UpdatePreyPosition] TrackingClient NULL"))
		return;
	}
	...
	// UE_LOG(LogExperiment, Log, TEXT("[AExperimentServiceMonitor::UpdatePreyPosition] Sent frame: %i"), FrameCountPrey)
	FrameCountPrey += 1;
}
...
bool AExperimentServiceMonitor::SetupPlayerUpdatePosition() {
	...
	if (Cast<APawnMain>(Pawn)) {
		...
		UE_LOG(LogExperiment, Log,
		       TEXT("[AExperimentServiceMonitor::SetupPlayerUpdatePosition()] APawnMain found and assigned."));
		PlayerPawn->MovementDetectedEvent.AddDynamic(this, &AExperimentServiceMonitor::UpdatePreyPosition);
	}
	else {
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::SetupPlayerUpdatePosition()] No valid pawn found nor assigned."));
		return false;
	}
...
void AExperimentServiceMonitor::SelfDestruct(const FString InErrorMessage) {
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green,
	                                              FString::Printf(
		                                              TEXT(
			                                              "[AExperimentServiceMonitor::SelfDestruct] Tracking and Experiment ABORTED. Something happened: %s"),
		                                              *InErrorMessage));
	UE_LOG(LogExperiment, Error,
	       TEXT("[AExperimentServiceMonitor::SelfDestruct()] Something went wrong. Destroying. Reason: %s "),
	       *InErrorMessage);
...
bool AExperimentServiceMonitor::SubscribeToTracking() {
	...
	TrackingSubscribeRequest->ResponseReceived.AddDynamic(
		this, &AExperimentServiceMonitor::HandleSubscribeToTrackingResponse);
...
void AExperimentServiceMonitor::RequestRemoveDelegates(URequest* InRequest, const FString& InRequestString) {
	...
	if (InRequest->IsValidLowLevelFast()) {
		...
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::RequestRemoveDelegates] Failed. RequestIn (%s) NULL"),
		       *InRequestString)
	}
	UE_LOG(LogExperiment, Log, TEXT("[AExperimentServiceMonitor::RequestRemoveDelegates] Request: %s; Result: %i"),
...
void AExperimentServiceMonitor::HandleGetOcclusionsResponse(const FString ResponseIn) {
	...
	if (GetOcclusionsRequest->IsValidLowLevelFast()) {
		UE_LOG(LogExperiment, Log,
		       TEXT(
			       "[AExperimentServiceMonitor::HandleGetOcclusionsResponse] RequestRemoveDelegates(GetOcclusionsRequest)"
		       ));
		...
		UE_LOG(LogExperiment, Error,
		       TEXT(
			       "[AExperimentServiceMonitor::HandleGetOcclusionsResponse] Failed to remove delegates, GetOcclusionsRequest not valid."
		       ));
	}
}
...
void AExperimentServiceMonitor::HandleGetOcclusionsTimedOut() {
	UE_LOG(LogExperiment, Warning,
	       TEXT("[AExperimentServiceMonitor::HandleGetOcclusionsTimedOut] Get occlusion request timed out!"));
	if (!GetOcclusionsRequest->IsValidLowLevelFast()) {
		UE_LOG(LogExperiment, Error,
		       TEXT(
			       "[AExperimentServiceMonitor::HandleGetOcclusionsTimedOut] Failed to remove delegates, GetOcclusionsRequest not valid."
		       ));
		return;
	}

	UE_LOG(LogExperiment, Log,
	       TEXT("[AExperimentServiceMonitor::HandleGetOcclusionsResponse] Removed delegates for: GetOcclusionsRequest"
	       ));
	this->RequestRemoveDelegates(GetOcclusionsRequest, "GetOcclusionsRequest");
}
...
bool AExperimentServiceMonitor::SendGetOcclusionLocationsRequest() {
	UE_LOG(LogExperiment, Log,
	       TEXT("[AExperimentServiceMonitor::SendGetOcclusionLocationsRequest()] Starting request."));
	if (!ensure(ValidateClient(TrackingClient))) {
		UE_LOG(LogExperiment, Error,
		       TEXT(
			       "[AExperimentServiceMonitor::SendGetOcclusionLocationsRequest()] Cant send get occlusion request, Experiment service client not valid."
		       ));
		return false;
	}
	...
	GetOcclusionLocationRequest->ResponseReceived.AddDynamic(
		this, &AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse);
	GetOcclusionLocationRequest->TimedOut.AddDynamic(
		this, &AExperimentServiceMonitor::HandleGetOcclusionLocationsTimedOut);
...
void AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse(const FString ResponseIn) {
	...
	if (!OcclusionsStruct.bSpawnedAll) {
		...
		UE_LOG(LogExperiment, Log,
		       TEXT(
			       "[AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse] All locations already spawned. Skipping spawn!"
		       ));
	}
	...
	if (!this->SendGetOcclusionsRequest()) {
		UE_LOG(LogExperiment, Error,
		       TEXT(
			       "[AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse] Failed to SendGetOcclusionsRequest"
		       ))
	}
	else {
		UE_LOG(LogExperiment, Log,
		       TEXT("[AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse] Sent SendGetOcclusionsRequest OK!"
		       ))
	}
}
...
void AExperimentServiceMonitor::HandleGetOcclusionLocationsTimedOut() {
	...
	if (OcclusionsStruct.bAllLocationsLoaded && OcclusionsStruct.bSpawnedAll) {
		...
		if (!this->SendGetOcclusionsRequest()) {
			...
			UE_LOG(LogExperiment, Log,
			       TEXT(
				       "[AExperimentServiceMonitor::HandleGetOcclusionLocationsResponse] Sent SendGetOcclusionsRequest OK!"
			       ))
		}
	}
}

bool AExperimentServiceMonitor::ConnectToServer(UMessageClient* ClientIn, const int MaxAttemptsIn,
	...
	while (AttemptCurr < MaxAttemptsIn) {
		if (ClientIn->Connect(IPAddressIn, PortIn)) {
			UE_LOG(LogExperiment, Log,
			       TEXT("[AExperimentServiceMonitor::ConnectToServer()] Success (attempt #: %i/%i)"), AttemptCurr + 1,
			       MaxAttemptsIn);
			break;
		}
		AttemptCurr += 1;
	...
	if (!ClientIn->IsConnected()) {
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::ConnectToServer()] Failed to connect to server!"));
		return false;
	}
...
bool AExperimentServiceMonitor::RoutePredatorMessages() {
	...
	if (!MessageRoutePredator) {
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::RoutePredatorMessages] MessageRoutePredator not valid."));
		return false;
	}
...
bool AExperimentServiceMonitor::SetupConnections() {
	...
	if (TrackingClient->IsValidLowLevelFast() && TrackingClient->IsConnected()) {
		...
		else { UE_LOG(LogExperiment, Log, TEXT("[AExperimentServiceMonitor::SetupConnections] disconnect OK")); }
	}
	...
	if (!this->ConnectToServer(TrackingClient, AttemptsMax, *ServerInfo.IP, ServerInfo.Port)) {
		UE_LOG(LogExperiment, Fatal,
		       TEXT("[AExperimentServiceMonitor::SetupConnections] Connect to Tracking: Failed! (IP: %s)"),
		       *ServerInfo.IP);
		return false;
	}

	UE_LOG(LogExperiment, Log,
	       TEXT("[AExperimentServiceMonitor::SetupConnections()] Connect to Tracking: OK! (IP: %s)"),
	       *ServerInfo.IP);
...
bool AExperimentServiceMonitor::Test() {
	...
	if (ExperimentManager) {
		...
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::Test] Failed to Register new player. ExperimentManager not valid!"))
	}

	/* get cell locations */
	if (!this->SendGetOcclusionLocationsRequest()) {
		UE_LOG(LogExperiment, Error,
		       TEXT("[AExperimentServiceMonitor::Test] Failed to SendGetOcclusionLocationsRequest"))
	}
...
void AExperimentServiceMonitor::SetOcclusionVisibility(const bool bNewVisibility) {
	UE_LOG(LogExperiment, Log,
		TEXT("[AExperimentServiceMonitor::SetOcclusionVisibility] Called with value: %i"), bNewVisibility)

	if (ensure(OcclusionsStruct.bCurrentLocationsLoaded)) {
		UE_LOG(LogExperiment, Log,
		       TEXT("[AExperimentServiceMonitor::SetOcclusionVisibility] Using last loaded locations (%i)"),
...
void AExperimentServiceMonitor::BeginPlay() {
	...
	if (this->SpawnAndPossessPredator()) { UE_LOG(LogExperiment, Log, TEXT("Spawned predator: OK")); }
		...
		UE_LOG(LogExperiment, Error, TEXT("Spawned predator: FAILED"));
		UE_DEBUG_BREAK();
	}
...
void AExperimentServiceMonitor::Tick(float DeltaTime) { Super::Tick(DeltaTime); }
